...
- 台湾華語→日本語の `resultText` が、友達との対面会話やSNS/DMにそのまま使える自然な口語か
- 中国語の語順や日本語で不要な主語を残していないか
- `friendly` は友達口調、`polite` は柔らかい丁寧語になっているか
- 原文より恋愛的、過度に丁寧、強制的になっていないか
- `literalMeaning` が原文に近い意味の補助情報として分離され、`resultText` と同じ役割になっていないか
- `literalMeaning` がtargetLanguageで自然に成立し、原文にない感情を足さず、一文または短い句に収まっているか
- 日本語補助文で `来年` など一般的な語を優先し、中国語の語順や不要な代名詞を残していないか
- 辞書的な置換ではなく、意図・距離感・感情を保った会話向けローカライズになっているか

### Nuance

- `nuance` が非空で、targetLanguageの自然な短文になっているか
- `resultText` や `literalMeaning` の複製ではないか
- friendly / polite / casualなどの距離感と、対面・メッセージでの使い方を補助しているか
- イベント会場だけに用途を限定していないか
...
### Natural Japanese cases

| ID | ケース | sourceText | tone | 期待 |
| --- | --- | --- | --- | --- |
| NJ01 | 友達への誘い | 下次也一起玩吧～ | friendly | 「次もまた一緒に遊ぼうね〜」方向の友達口調。硬い「次回も一緒に遊びましょう」を避ける。 |
| NJ02 | 写真のお礼 | 謝謝你幫我拍照！ | friendly | 「写真撮ってくれてありがとう！」方向。説明を足しすぎない。 |
| NJ03 | 来年も来て | 明年也要來喔！ | friendly | 「来年も来てね！」方向。義務表現にしない。 |
| NJ04 | 写真を送る | 等一下我把照片傳給你～ | friendly | 「あとで写真送るね〜」方向。不自然な主語を残さない。 |
| NJ05 | 丁寧トーン | 今天真的很開心！ | polite | 「今日は本当に楽しかったです！」方向。ビジネス敬語にしない。 |
| LM01 | 来年表現 | 明年也要來喔！ | friendly | `resultText` は友達向け、`literalMeaning` は「来年も来てくださいね」系。`明年も` はfailではなくwarn。 |
| LM02 | 写真送付 | 等一下我把照片傳給你～ | friendly | `resultText` は会話文、`literalMeaning` は「あとで写真を送ります」系。不自然な中国語語順を避ける。 |
...
- API 500 は fail。
- `resultText` 空は fail。
- `literalMeaning` 空は fail。
- `nuance` 空、targetLanguage不一致、`resultText` / `literalMeaning` との完全一致は fail。
...
- 意図キーワード不足は warn。
- `resultText` と `literalMeaning` が完全一致し、役割分離できていない場合は fail。
- targetLanguageが日本語なのに `literalMeaning` が日本語らしくない場合は fail。
- `literalMeaning` の `明年も`、`少し後で私は`、`あなたに送信します`、`することができます`、`しなければなりません`、`してくださいませ` はwarn候補。自然さの自動確定には使わない。
- `次回も`、`することができます`、`あなたに送信します`、`本日は`、`しなければなりません`、`してくださいませ`、`撮影を手伝ってくれて` は文脈依存のwarn候補。自動failにはしない。
- 自然さは自動で確定しない。
