...
- TTSと端末音声がともに失敗した場合、エラー後にボタンが `聞く` / `ゆっくり` へ戻り、再度操作できる。
- `自然・やわらかめ` と `自然・落ち着いた声` で、実際の声の印象に違いがあるか人間が聞き比べる。
- 台湾華語が不自然に硬くなく、通常 / ゆっくりの差が分かり、ゆっくりでも自然な抑揚を保つか確認する。
- 文の感情・距離感と声のトーンが合っているか確認する。
- 読み上げ中に録音を開始すると、読み上げが止まってから録音中になる。
- 録音中は `聞く` / `ゆっくり聞く` が無効になり、端末音声やAI音声がマイクへ混ざらない。
- 録音停止後は読み上げボタンが再び操作できる。
...
- `literalMeaning` は自然な対象言語で、`resultText` の複製や機械翻訳的な語順になっていない。
- `nuance` は距離感・トーン・使い方の補助情報で、`resultText` と `literalMeaning` の繰り返しになっていない。
- 来年の文では、会話UIとして一般的な `来年` が使われ、`明年` が残っていないか確認する。
